...
function decodeAnyValue(anyValue: OtlpJsonAnyValue | undefined): unknown {
  ...
  if (anyValue.kvlistValue) {
    return Object.fromEntries(
      (anyValue.kvlistValue.values ?? []).map((kv) => [
        kv.key,
        decodeAnyValue(kv.value),
      ])
    );
  }
  return null;
...
export function severityColorClass(n: SeverityNumber): string {
  ...
  if (n >= SeverityNumber.SEVERITY_NUMBER_WARN) return "text-yellow-400";
  if (n >= SeverityNumber.SEVERITY_NUMBER_INFO) return "text-green-400";
  if (n >= SeverityNumber.SEVERITY_NUMBER_DEBUG) return "text-blue-400";
  if (n >= SeverityNumber.SEVERITY_NUMBER_TRACE) return "text-muted-foreground";
